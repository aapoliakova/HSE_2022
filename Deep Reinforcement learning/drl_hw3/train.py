...
class TD3:
    ...
    def update(self, transition):
        ...
        if len(self.replay_buffer) > BATCH_SIZE * 16:
            ...
            # a’ := 𝛑(s’) + clip(ε’, -c, c), ε’ ~ N(0, 𝞂’)
            with torch.no_grad():
                 target_action = self.target_actor(next_state)
                 target_action = torch.clip(target_action + torch.clip(sigma * torch.randn_like(target_action), -c, c), -1, 1)
    ...
    def save(self):
        torch.save(self.actor, path + "agent.pkl")
